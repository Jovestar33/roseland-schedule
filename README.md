...
Start with [READINESS.md](./READINESS.md) for dated verified branch status, validation scope and superseded historical entries; use the September 16 audit for current gates and proposed next work. The heavy-usage freeze has ended; isolated restart fixes and local verification are recorded there; new hosted migrations, merge and deployment remain unauthorized.

Keep this local project’s notes current as work proceeds. READINESS records both foundation drafts, recovered conversation context, missing provenance, and the exact checkout containing this documentation refresh.

[Architecture](./ARCHITECTURE.md) · [Roadmap](./ROADMAP.md) · [Stability policy](./PRODUCTION_STABILITY_POLICY.md) · [Platform plan](./PLATFORM_MIGRATION_PLAN.md)

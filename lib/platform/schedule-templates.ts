...
export function createTemplateRepository(client:SupabaseClient){
 ...
 return {
  ...
  async reviewApply(actor:string,id:string,target:string,organization:string,purpose:'apply'|'export'='apply'):Promise<TemplateApplyReview>{const v=await rpc(actor,purpose==='export'?'review_schedule_template_export':'review_schedule_template_apply',{target_template_id:parseInvitationId(id),target_schedule_id:parseInvitationId(target)});const t=record(v?.template);if(t.id!==id||t.organization_id!==organization||!Array.isArray(t.rows)||v.target_id!==target||!Number.isSafeInteger(v.target_version)||v.target_version<1||typeof v.policy!=='string'||!v.policy)invalid();return v;},

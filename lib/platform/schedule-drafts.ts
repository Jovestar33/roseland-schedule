...
export interface DraftBackup {actor:string;organization:string;source:string;before:StoredSchedule;document:StoredSchedule['document'];undo:unknown[];redo:unknown[];savedAt:number}
...
export function readSourceDraft(storage:DraftStorage,actor:string,organization:string,source:string):DraftBackup|null{
 ...
 validateFileDocument(v.document);for(const entry of [...v.undo,...v.redo])validateFileDocument(entry);

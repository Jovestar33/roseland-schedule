...
## Organization presentation authority — confirmed 2026-09-17

The user explicitly assigned organization-wide labels, theme colors and action styling to **Organization Super Admins and Admins only**. Existing logo/action configuration follows that same organization settings record. Active members may read the shared presentation; Production Organizers, Editors and Viewers cannot write it merely through production membership. This authority is distinct from source Production Organizers' approved template-publication authority. Versioned local implementation and acceptance are recorded in [ORGANIZATION_PRESENTATION_CHECKPOINT.md](./ORGANIZATION_PRESENTATION_CHECKPOINT.md).

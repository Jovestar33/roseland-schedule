...
function setup({ failWrite = false } = {}) {
  ...
  const context = {
    ...
    require: name => name === '@netlify/blobs' ? {
      connectLambda() {},
      getStore: options => blobs.getStore({ ...options, siteID: 'synthetic', token: 'synthetic', edgeURL: 'https://synthetic.invalid' }),
    } : require(name),
    fetch: async (url, options) => {
      assert.equal(new URL(url).hostname, 'synthetic.invalid');
      const key = new URL(url).pathname;
      if (options.method.toLowerCase() === 'get') {
        return entries.has(key) ? new Response(entries.get(key)) : new Response(null, { status: 404 });
      }
      ...
      return new Response(null, { status: 200, headers: { etag: 'synthetic-etag' } });
    },
  };

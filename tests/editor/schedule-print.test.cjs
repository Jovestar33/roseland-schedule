...
test('legacy shared views retain key instructions without exposing private row fields',()=>{
 const {publicSchedule}=require('../../netlify/lib/public-view.js');
 const data=fixture();data.rows[0].status='Private';data.rows[0].contactEmail='private@example.invalid';
 const result=publicSchedule(data);assert.equal(result.rows[0].keyInstruction,data.rows[0].keyInstruction);
 assert.equal(result.rows[0].status,undefined);assert.equal(result.rows[0].contactEmail,undefined);
});

...
interface Props {embedded?:boolean;onClose?:()=>void;openRequest?:number;client:SupabaseClient;actor:string|null;organization:string|null;enabled:boolean;canWriteCurrent:boolean;getSource:()=>StoredSchedule|null;canApply:()=>boolean;onApply:(review:TemplateApplyReview,context:TemplateDraftContext)=>boolean;onState:(dirty:boolean,busy:boolean)=>void;requireAuth:()=>void}
export default function LocalScheduleTemplates({embedded=false,onClose,openRequest,client,actor,organization,enabled,canWriteCurrent,getSource,canApply,onApply,onState,requireAuth}:Props){
 ...
 const reviewFooter=apply?<><button className="btn btn-primary" disabled={busy} onClick={()=>void run(async current=>{const fresh=await repo.reviewApply(actor!,apply.review.template.id,apply.context.id,organization!);if(!current())return;if(fresh.template.version!==apply.review.template.version||fresh.policy!==apply.review.policy||fresh.target_version!==apply.context.version)throw Error('The template, source permissions or receiving schedule changed. Cancel and review again; your draft is retained.');if(!onApply(fresh,apply.context))throw Error('The current document changed during review. Cancel and review again; your newer edits are retained.');setApply(null);setOpen(false);onClose?.();setMessage('Template applied to the draft. Metadata is retained. Undo is available; Save schedule commits it.');})}>Apply rows to draft</button><button className="btn btn-light" disabled={busy} onClick={()=>{setApply(null);setMessage('Apply cancelled. Your draft is unchanged.');}}>Keep current rows</button></>:pending?<div><button className="btn btn-primary" disabled={busy} onClick={()=>void run(current=>send(current))}>{pending.started?'Retry exact template request':pending.attempt.operation==='publish'?'Confirm publish to organization':'Confirm template change'}</button>
    {pending.started?<><button className="btn btn-light" disabled={busy} onClick={()=>void run(current=>send(current,true))}>Check template result</button><p>The exact request is retained for this account and organization, including after reload. An unconfirmed result does not mean it failed.</p></>:<button className="btn btn-light" disabled={busy} onClick={()=>{clearTemplateRequest(localStorage,pending.attempt);setPending(null);setMessage('Review cancelled. Nothing was sent.');}}>Cancel template change</button>}</div>:undefined;
 return <section aria-label="Templates">
  {!embedded&&<button className="btn btn-light" disabled={!enabled||busy||!actor||!organization} onClick={()=>{visibility.current++;setOpen(true);void run(current=>refresh(current));}}>Templates{inScope&&pending?' · request retained':''}</button>}
  {!embedded&&!open&&inScope&&message&&<p role="status">{message}</p>}
  <ModalVisibilityContext.Provider value={enabled&&inScope}><Modal open={open} onClose={()=>{visibility.current++;setOpen(false);onClose?.();}} title="Templates" className="template-modal" footer={reviewFooter} retainWhenHidden>
   <p className={styles.intro}>Browse reusable schedule rows for your production, or templates shared across your organization.</p>
   <p hidden={!message} role="status" aria-live="polite" className={styles.feedback}>{message}</p>
   ...
   <fieldset hidden={!!pending||!!apply} disabled={busy||!!pending||!!apply} style={{border:0,padding:0}}>
    ...
    <button className="btn btn-light" onClick={()=>void run(current=>refresh(current))}>Refresh templates</button></div>

    {!items.length&&<p>No available templates in this library.</p>}
    <ul className={styles.list} aria-label="Available templates">{items.map(t=><li key={t.id}><button className={styles.item} aria-label={t.name} aria-pressed={selected?.id===t.id} onClick={()=>void run(async current=>{const v=await repo.read(actor!,t.id,organization!);if(current()){setSelected(v);setName(v.name);}})}><span><strong>{t.name}</strong><small>{t.row_count} {t.row_count===1?'row':'rows'} · {t.published_at?'Shared with organization':'Production only'}{t.production_name?` · ${t.production_name}`:''}</small></span><span aria-hidden="true">›</span></button></li>)}</ul>
   </fieldset>

   {!pending&&!apply&&selected&&<fieldset disabled={busy} className={styles.selected}><legend>{selected.name}</legend>
    <details className={styles.preview} open><summary>Review all template rows, including notes and contacts</summary><TemplateRowsPreview rows={selected.rows??[]}/></details>
    ...
    {(selected.can_manage||selected.can_trash||selected.can_publish)&&<details className={styles.manage}><summary>Manage name, sharing and Trash</summary>
    ...
    {!pending&&!apply&&mode==='production'&&writeProduction&&(canCapture?<details className={styles.create}><summary>Save rows from the current schedule as a template</summary><div><label>New template name<input disabled={busy} value={newName} onChange={e=>setNewName(e.target.value)} maxLength={150}/></label><button className="btn btn-primary" disabled={busy||!newName.trim()||!canCapture} onClick={()=>void run(async()=>{const s=getSource();if(!s||!canCapture)return;begin(captureTemplateAttempt({actor:actor!,organization:organization!,production,id:crypto.randomUUID(),request:crypto.randomUUID(),version:0,operation:'create',name:newName.trim(),rows:templateDraftRows(useScheduleStore.getState().rows),source:s.id,sourceVersion:s.document_version,previewName:newName.trim(),previewRows:templateDraftRows(useScheduleStore.getState().rows)}));})}>Save draft as template</button></div></details>:<p className={styles.hint}>{pendingUses?'Save your applied template changes before creating another template.':'Open an editable schedule in this production to save its rows as a new template.'}</p>)}
   ...
   {pending&&<section aria-label="Template change review"><h3>{operationLabel[pending.attempt.operation]}: {pending.attempt.previewName}</h3><p>Destination production: {productions.find(p=>p.id===pending.attempt.production)?.name??'Original reviewed production'}</p>
    ...
    <details open={pending.attempt.operation==='publish'}><summary>Review exact rows ({pending.attempt.previewRows?.length??0})</summary><TemplateRowsPreview rows={pending.attempt.previewRows??[]}/></details>
   ...
   {apply&&<section aria-label="Apply template review"><h3>Apply {apply.review.template.name}</h3><p>Replace the current rows, including any unsaved row edits. Keep this schedule’s metadata, name and placement. You can Undo the application. Save schedule commits the rows and their source restrictions together.</p>
    <details><summary>Review rows to apply ({apply.review.template.rows.length})</summary><TemplateRowsPreview rows={apply.review.template.rows}/></details>

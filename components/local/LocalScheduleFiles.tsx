...
interface Props {client:SupabaseClient;actor:string|null;organization:string|null;enabled:boolean;copyEnabled:boolean;getDraft:()=>StoredSchedule['document'];getSource:()=>StoredSchedule|null;name:string;onState:(dirty:boolean,busy:boolean)=>void;requireAuth:()=>void}
export default function LocalScheduleFiles({client,actor,organization,enabled,copyEnabled,getDraft,getSource,name,onState,requireAuth}:Props){
  ...
  return <section className={styles.files} aria-label="Schedule files">
    ...
    <div className={styles.toolbar}>
      ...
      <button className="btn btn-light" disabled={!enabled||!copyEnabled||!organization||busy||entries.length>0||!!review} onClick={()=>{const data=structuredClone(getDraft()),record=getSource();if(!record)return;void run(async current=>{const policy=await repo.copyPolicy(actor!,record.id);if(!current())return;setSource({id:record.id,version:record.document_version,policy});await loadEntries([{name:`${name} copy`,slug:suggestedSlug(`${name} copy`),data}],current);});}}>Copy / Save As current draft</button>

...
interface Props {client:SupabaseClient;actor:string|null;organization:string|null;enabled:boolean;copyEnabled:boolean;getDraft:()=>StoredSchedule['document'];name:string;onState:(dirty:boolean,busy:boolean)=>void;requireAuth:()=>void}
export default function LocalScheduleFiles({client,actor,organization,enabled,copyEnabled,getDraft,name,onState,requireAuth}:Props){
  ...
  async function loadEntries(next:FileEntry[],current:()=>boolean){const d=await repo.destinations(actor!,organization!);if(!current())return;setEntries(next);setReview(null);setReviewOrg(organization);setDays(d);setDay(d[0]?.id??'');setMessage(d.length?'Review names, slugs, destination and complete documents before creating.':'No existing days are editable by this account. Export remains available.');}
  function clear(){setEntries([]);setReview(null);setReviewOrg(null);setMessage('Review closed. Your source draft is unchanged.');if(fileRef.current)fileRef.current.value='';}
  return <section className={styles.files} aria-label="Schedule files">
    <h2>Copy and schedule files</h2>
    <p>Schedule-only JSON includes saved documents readable by your account across organizations, including readable archived/deleted schedules. It excludes history, snapshots, templates, CMS, memberships, hierarchy definitions, aliases and browser-only data. Unsaved edits stay in this tab.</p>
    <div className={styles.toolbar}>
      ...
      <button className="btn btn-light" disabled={!enabled||!copyEnabled||!organization||busy||entries.length>0||!!review} onClick={()=>{const data=structuredClone(getDraft());void run(current=>loadEntries([{name:`${name} copy`,slug:suggestedSlug(`${name} copy`),data}],current));}}>Copy / Save As current draft</button>
      <label>Import schedule-only JSON<input ref={fileRef} type="file" accept=".json,application/json" disabled={!enabled||!organization||busy||entries.length>0||!!review} onChange={e=>{const file=e.target.files?.[0];if(!file)return;void run(async current=>{if(file.size>FILE_BYTES)throw new Error('File exceeds 20 MB.');setMessage(`Reading ${file.name}…`);const contents=await file.text();const next=await parseScheduleFile(contents,file.name.replace(/\.json$/i,''));await loadEntries(next,current);});e.target.value='';}}/></label>
    </div>
    ...
    {inScope&&entries.length>0&&!review&&<fieldset disabled={!enabled||busy}>
      ...
      <p>All items become new drafts with fresh IDs in the selected existing day. Source IDs, status, deleted state and hierarchy are not imported. Document metadata stays exactly as supplied. Existing schedules are never overwritten.</p>
      <label>Destination production day<select value={day} onChange={e=>setDay(e.target.value)}><option value="">Choose an editable day</option>{days.map(d=><option key={d.id} value={d.id}>{d.label}</option>)}</select></label>
      ...
      <button className="btn btn-primary" disabled={!day} onClick={()=>void run(async current=>{const proposed=new ScheduleFileImport(actor!,organization!,day,entries);await repo.preflight(actor!,organization!,day,entries);if(current()){setReview(proposed);setMessage('Review locked. Confirm to create these exact documents; later source edits remain in your draft.');}})}>Prepare create-only review</button>
    ...
    {inScope&&review&&<div className={styles.recovery}>
      ...
      <p>Destination: {days.find(d=>d.id===review.day)?.label}. {review.items.filter(c=>c.phase==='success').length} of {review.items.length} confirmed. Keep this tab for recovery; closing or reloading loses request details. Download the receipt to retain IDs.</p>
      <ul>{review.receipt().map(r=><li key={r.id}>{r.name} · {r.state}{r.failure?` (${r.failure})`:''}<br/><small>{r.id}</small></li>)}</ul>
      <button className="btn btn-primary" disabled={!enabled||busy||review.complete} onClick={()=>void run(async current=>{await review.execute(repo.lifecycle,current);if(current()){setMessage(review.complete?'Creation confirmed. Source and newer draft edits are unchanged.':'Stopped at an unconfirmed item. Check the original IDs before retrying.');if(review.items.some(c=>c.failure==='unauthenticated'))requireAuth();}})}>{review.started?'Retry unconfirmed original IDs':'Confirm create new schedules'}</button>
      {review.started&&!review.complete&&<button className="btn btn-light" disabled={!enabled||busy} onClick={()=>void run(async current=>{await review.execute(repo.lifecycle,current,true);if(current())setMessage('Original IDs checked. Review the recorded results.');})}>Check original IDs</button>}

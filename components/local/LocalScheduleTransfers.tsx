...
export default function LocalScheduleTransfers({client,actor,organization,enabled,request,onState,getDraft,captureOpen,onCopy}:Props){
 ...
 async function begin(id:string,copy:boolean,g:number){
  ...
  const dest=destinations.find(v=>v.id===saved.production_id)??destinations[0];setDestination(dest?.id??'');const sourcePhase=saved.phase_id??p.find(v=>v.id===saved.production_id)?.days.find(d=>d.id===saved.production_day_id)?.phase;setPhase(sourcePhase&&p.find(v=>v.id===dest?.id)?.phases.some(f=>f.id===sourcePhase)?sourcePhase:'');setDay('');setPreview(null);setAttempt(null);setComplete(false);setMessage(destinations.length?'Choose a destination. Leaving the day unassigned keeps the document date.':copy?'No production is available where you can create a copy.':'No other production is available to this account for this move. Your schedule stays in its current production.');
 }
 ...
 {attempt&&<><p>{{copy:'Duplicate',move:'Move',request:'Request',approve:'Approval'}[attempt.kind]} · saved version {attempt.version} · {placement?.name??choices.find(p=>p.id===destination)?.name??incoming?.destination} · {attempt.day?'Selected day':'Unassigned'} · {placement?.phases.find(f=>f.id===attempt.phase)?.name??'No phase'}. {attempt.kind==='copy'&&<>Copy name: {attempt.name} · Address: {attempt.slug}. </>}Date, location and rows are retained; Move and Duplicate use destination production/phase labels.</p>{!complete&&<button disabled={busy} onClick={()=>void run(confirm)}>Confirm or recover {{copy:'Duplicate',move:'Move',request:'request',approve:'approval'}[attempt.kind]}</button>}{complete&&<p>Confirmed identity: {attempt.kind==='copy'?attempt.id:attempt.source}</p>}</>}

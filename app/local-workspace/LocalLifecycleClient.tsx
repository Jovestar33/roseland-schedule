...
import ScheduleDocumentPreview from '@/components/local/ScheduleDocumentPreview';
...
export default function LocalLifecycleClient(){
  ...
  useEffect(()=>{if(workspace.review&&workspace.active)historyHeading.current?.focus();},[workspace.review,workspace.active,record?.id]);
  ...
  if(workspace.review)return <main className={lifecycleStyles.history}>
    <header className={lifecycleStyles.historyHeader}>
      <div><button className="btn btn-light btn-sm" disabled={busy} onClick={()=>workspace.openOrganization(workspace.organization!.id)}>← Back to schedules</button><h1 ref={historyHeading} tabIndex={-1}>Schedule history</h1><p>{record?.display_name??'Choose a schedule to review its saved versions.'}</p></div>
      {record&&<div className={lifecycleStyles.headerActions}><span className={lifecycleStyles.versionBadge}>Current version {record.document_version}</span><button className="btn btn-light" disabled={!ready||busy||!!record.deleted_at} onClick={()=>workspace.openSchedule(workspace.organization!.id,record.id)}>Open schedule</button><button className="btn btn-light btn-sm" disabled={!ready||busy||!!attempt} onClick={()=>choose(record.id)}>Refresh history</button></div>}
    </header>
    <p hidden={!busy&&['Choose a saved schedule or create a blank schedule in an authorized day.','Saved schedule loaded. The editor draft is unchanged.'].includes(message)} role="status" aria-live="polite" className={lifecycleStyles.feedback}>{busy?'Loading or reviewing…':message}</p>
    {!record&&<section className={lifecycleStyles.chooseSchedule} aria-label="Choose schedule history"><h2>Saved schedules</h2>{items.map(item=><button key={item.id} className="btn btn-light" disabled={!ready||busy} onClick={()=>workspace.openLifecycle?.(workspace.organization!.id,item.id)}>{item.display_name}<small>Version {item.document_version}</small></button>)}{more&&<button className="btn btn-light" disabled={!ready||busy} onClick={()=>void run(()=>list(filter,true))}>Load more schedules</button>}</section>}
    {record&&<>
      <p className={lifecycleStyles.explanation}>Preview a saved version before restoring it. Restoration creates a new version; your schedule’s name, placement and earlier history stay intact.</p>
      <div className={lifecycleStyles.historyLayout}>
        <nav aria-label="Saved versions" className={lifecycleStyles.versions}>{history.map(item=><button key={item.version} className={lifecycleStyles.version} aria-current={preview?.version===item.version?'true':undefined} disabled={!ready||busy||!!attempt} onClick={()=>void run(async()=>{const value=await repo.historical(actor.current!,workspace.organization!.id,record.id,item.version);if(!value)throw new ScheduleRepositoryError('unavailable');setPreview(value);})}><strong>Version {item.version}{item.version===record.document_version&&<span>Current</span>}</strong><time>{new Date(item.created_at).toLocaleString()}</time></button>)}{historyMore&&<button className="btn btn-light btn-sm" disabled={!ready||busy||!!attempt} onClick={()=>void run(()=>loadHistory(record,true))}>Load older versions</button>}</nav>
        <section aria-label="Historical document preview" className={lifecycleStyles.document}>
          {preview?<><div className={lifecycleStyles.documentHeader}><div><h2>Version {preview.version}</h2><p>{new Date(preview.created_at).toLocaleString()}</p></div><button className="btn btn-primary" disabled={!ready||busy||!!attempt||!canEdit||!!record.deleted_at||preview.version===record.document_version} onClick={()=>prepare('restore_version')}>Review restoration</button></div><ScheduleDocumentPreview document={preview.document}/></>:<div className={lifecycleStyles.empty}><h2>Select a version</h2><p>Review the complete schedule, including row notes, contacts and call-sheet details.</p></div>}
        </section>
      </div>
    </>}
    {attempt&&<section ref={review} tabIndex={-1} className={lifecycleStyles.restoreReview} aria-label="History restoration review"><h2>{controller.phase==='success'?'Restoration confirmed':'Restore this version?'}</h2><p>Restore version {Number(attempt.payload.version)} as a new saved version of <strong>{attempt.name}</strong>. Your unsaved editor draft is retained separately.</p>{controller.failure&&<p role="alert">{messages[controller.failure]}</p>}{controller.result?.state==='matched'&&<p>Saved as version {controller.result.matchedVersion}. All earlier versions remain available.</p>}<div className={lifecycleStyles.reviewActions}>{controller.phase==='success'?<button className="btn btn-primary" disabled={busy} onClick={()=>{controller.clear();refresh();setMessage('Restoration complete. Choose Open schedule to view the saved result.');}}>Done</button>:<><button className="btn btn-primary" disabled={!ready||busy||controller.phase==='conflict'} onClick={()=>void execute()}>{controller.phase==='review'?'Confirm restoration':'Retry original restoration'}</button><button className="btn btn-light" disabled={!ready||busy} onClick={()=>void execute(true)}>Check saved result</button><button className="btn btn-light" disabled={busy} onClick={discard}>{controller.phase==='review'?'Cancel restoration':'Discard local review'}</button></>}</div></section>}
    <dialog ref={dialog} className={lifecycleStyles.confirmation} aria-label={confirmation?.title??'Confirm history action'} onCancel={()=>setConfirmation(null)}><h2>{confirmation?.title}</h2><p>{confirmation?.text}</p><div className={lifecycleStyles.reviewActions}><button className="btn btn-light" onClick={()=>setConfirmation(null)}>Keep working</button><button className="btn btn-primary" onClick={()=>{const action=confirmation?.action;setConfirmation(null);action?.();}}>{confirmation?.label}</button></div></dialog>
  </main>;

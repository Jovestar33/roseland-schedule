...
export default function LocalLifecycleClient(){
  ...
  const [permission,setPermission]=useState<{id:string;token:string;allowed:boolean}|null>(null);
  ...
  const admin=!!workspace.organization&&workspace.organization.role!=='member';
  ...
  async function access(value:StoredSchedule){const stamp=generation.current,token=workspace.session?.access_token;const allowed=await repo.canEdit(actor.current!,value.production_id);if(stamp===generation.current&&token)setPermission({id:value.id,token,allowed});}
  ...
  return <div className={styles.page}><main className={styles.main}>
    ...
    <div className={styles.columns}>
      ...
      <section className={styles.card} aria-label="Saved schedule discovery"><div className={styles.sectionTitle}><h2>Saved schedules</h2><button className={styles.secondary} disabled={!ready||busy} onClick={()=>void run(()=>list())}>Refresh schedules</button></div>
        <label>Schedule collection<select disabled={!ready||busy} value={filter} onChange={event=>{const value=event.target.value as LifecycleFilter;setFilter(value);void run(()=>list(value));}}><option value="active">Active</option><option value="archived">Archived</option><option value="deleted" disabled={!admin}>Deleted · Owner/Admin</option></select></label>
    ...
    {record&&<section className={styles.card} aria-label="Saved schedule details"><div className={styles.sectionTitle}><h2>{record.display_name}</h2><span className={styles.badge}>{record.deleted_at?'Deleted':record.status} · version {record.document_version}</span></div><p className={styles.identifier}>Schedule: {record.id}</p><p>Address name: {record.slug}</p>
      ...
      {!record.deleted_at&&<form className={styles.form} onSubmit={event=>{event.preventDefault();prepare('rename');}}><fieldset disabled={!ready||busy||!!attempt||!canEdit}><label>Schedule name<input required maxLength={160} value={renameName} onChange={event=>setRenameName(event.target.value)}/></label><label>Address name<input required maxLength={160} pattern="[a-z0-9]+(-[a-z0-9]+)*" value={renameSlug} onChange={event=>setRenameSlug(event.target.value)}/></label></fieldset><button disabled={!ready||busy||!!attempt||!canEdit}>Review rename</button></form>}
      <div className={styles.actions}>
        {!record.deleted_at&&<button className={styles.secondary} disabled={!ready||busy||!!attempt||!canEdit||(record.status==='archived'&&!record.archived_from_status)} onClick={()=>prepare(record.status==='archived'?'unarchive':'archive')}>{record.status==='archived'?'Review unarchive':'Review archive'}</button>}
        {admin&&<button className={styles.secondary} disabled={!ready||busy||!!attempt||!canEdit} onClick={()=>prepare(record.deleted_at?'restore':'delete')}>{record.deleted_at?'Review recovery':'Review soft deletion'}</button>}
      </div>
    ...
    {record&&<section className={styles.card} aria-label="Document history"><h2>Document history</h2><p>Restoring appends the selected document as a new version. Current name, identity, archive and deletion state stay unchanged. Editor drafts are retained separately.</p><ul>{history.map(item=><li key={item.version}><strong>Version {item.version}</strong><p>{new Date(item.created_at).toLocaleString()} · {item.metadata?'Document and metadata recorded':'Older document-only history'}</p><button className={styles.secondary} disabled={!ready||busy||!!attempt} onClick={()=>void run(async()=>{const value=await repo.historical(actor.current!,workspace.organization!.id,record.id,item.version);if(!value)throw new ScheduleRepositoryError('unavailable');setPreview(value);})}>Preview version {item.version}</button></li>)}</ul>{historyMore&&<button disabled={!ready||busy||!!attempt} onClick={()=>void run(()=>loadHistory(record,true))}>Load older versions</button>}
      {preview&&<section aria-label="Historical document preview"><h3>Document from version {preview.version}</h3><div style={{background:'white',color:'#173848',padding:16,borderRadius:10,overflow:'auto'}}><ScheduleReadView data={preview.document as ScheduleData} name={record.display_name}/></div><button disabled={!ready||busy||!!attempt||!canEdit||!!record.deleted_at} onClick={()=>prepare('restore_version')}>Review document restore</button></section>}
    </section>}
    {attempt&&<section ref={review} tabIndex={-1} className={`${styles.card} ${styles.review}`} aria-label="Lifecycle request review"><div className={styles.sectionTitle}><h2>{labels[attempt.kind]}</h2><span className={styles.badge}>{controller.phase}</span></div><p>{workspace.organization?.name}</p><p className={styles.identifier}>Schedule: {attempt.id}</p><p>{attempt.kind==='create'?'Creation requires this identity to be absent.':`Expected saved version: ${attempt.expectedVersion}`}</p>
      ...
      {attempt.kind==='delete'&&<p>Move this saved schedule to Deleted. An Owner/Admin can recover it if its parents and address name remain available.</p>}

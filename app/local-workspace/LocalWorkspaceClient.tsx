...
import { LocalWorkspaceContext, type WorkspacePanelState } from '@/components/local/LocalWorkspaceContext';
...
export default function LocalWorkspaceClient({config}:{config:LocalEditorConfig}){
  ...
  async function navigate(next:WorkspaceLocation,push=true){
    ...
    try{
      ...
      if(next.organization&&!organization){setMessage('That organization is unavailable to this account. Retained drafts have not been discarded.');next={screen:next.screen==='acceptance'?'acceptance':'schedule',organization:null};}
      ...
      if(organization){remember(organization);setOrganizations(previous=>previous.map(item=>item.id===organization.id?organization:item));if(next.screen==='invitations')setManagementPanels(previous=>previous.includes(organization.id)?previous:[...previous,organization.id]);}
      setScope(organization);locationRef.current=next;setLocation(next);
      if(push)history.pushState(null,'',workspaceHref(next));else history.replaceState(null,'',workspaceHref(next));
    }catch(error){
  ...
  useEffect(()=>{
    ...
    const subscription=client.auth.onAuthStateChange((_event,next)=>{
      ...
      if(next&&identity.bind(next.user.id)){
        navigationTicket.current++;directoryTicket.current++;useScheduleStore.getState().newSchedule();setPanels({});setVisited([]);setManagementPanels([]);setOrganizations([]);setScope(null);setMore(false);setAccountEpoch(identity.generation);
      }
  ...
  const panel=(id:string,organization:WorkspaceOrganization|null,enabled:boolean)=>({client,session,authNeeded,organization,active:enabled,panelId:id,report,requireAuth,openOrganization});
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.shell}>
      ...
      {identity.actor&&<><div className={styles.account}><span>{session?.user.email??email}{authNeeded?' · Sign-in required':''}</span><span>{dirty?'Unfinished work retained':'No unfinished work'}</span><button disabled={busy||working} onClick={()=>dirty?setConfirmSignOut(true):void signOut()}>Sign out</button></div>
        <div className={styles.directory}><label>Workspace organization<select value={scope?.id??''} disabled={busy||authNeeded||!session} onChange={event=>void navigate({screen:active,organization:event.target.value||null})}><option value="">Choose an organization</option>{scope&&!organizations.some(item=>item.id===scope.id)&&<option value={scope.id}>{scope.name} · {scope.role}</option>}{organizations.map(item=><option key={item.id} value={item.id}>{item.name} · {item.role}</option>)}</select></label><button disabled={busy||authNeeded||!session} onClick={()=>void loadOrganizations().then(current=>current?navigate(locationRef.current,false):undefined).catch(()=>setMessage('Organizations could not be refreshed.'))}>Refresh organizations</button>{more&&<button disabled={busy||authNeeded||!session} onClick={()=>void loadOrganizations(true).catch(()=>setMessage('More organizations could not be loaded.'))}>Load more organizations</button>}</div>
        <nav aria-label="Workspace screens" className={styles.tabs}><button aria-current={active==='schedule'?'page':undefined} onClick={()=>tab('schedule')}>Schedules</button><button aria-current={active==='invitations'?'page':undefined} disabled={!scope||scope.role==='member'} onClick={()=>tab('invitations')}>Manage invitations</button><button aria-current={active==='acceptance'?'page':undefined} onClick={()=>tab('acceptance')}>Accept invitation</button></nav>
      </>}
    </div>
    ...
    <dialog ref={dialog} className={styles.dialog} onCancel={()=>setConfirmSignOut(false)} aria-label="Clear workspace"><h2>Sign out and clear all local work?</h2><p>Unsaved schedule edits, invitation drafts and pending request details will be cleared. A request already sent may have completed; clearing this workspace does not undo it.</p><div><button disabled={busy||working} onClick={()=>setConfirmSignOut(false)}>Keep working</button><button disabled={busy||working} onClick={()=>void signOut()}>Sign out and clear</button></div></dialog>
  </div>;
}

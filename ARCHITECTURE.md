...
| `components/toolbar/ShareDropdown.tsx` | Share/Print/Export dropdown; includes JSON export of the current schedule, Print/PDF, Contact Sheet, Call Sheet, and Team/Client link options; same fixed-position escape pattern |
| `components/modals/CallSheetModal.tsx` | Call Sheet modal and print portal (Phase 10); read-only schedule data + optional `meta.callsheet` fields edited inline; `LocationField` wraps `PlacesAutocomplete` with map-pin links for Basecamp/Parking/Hospital; `body.callsheet-printing` print isolation |
...
**Snapshot storage model (Phase 8)** — Snapshots are stored in Netlify Blobs (`schedule-snapshots` store); key = `snapshots_` + sha256(scheduleName). Shape: `{ id: UUID, savedAt: timestamp, label: string, data: ScheduleData }`. Server-side `normalizeSnapshots()` in `snapshots.js` deduplicates, sorts newest-first, and caps at **25 per schedule** (increased from 10 in Phase 8). Three creation paths: (1) **auto-snapshot** — 5-minute interval, always labeled `'Auto snapshot'`; failure is silent (best-effort); (2) **manual snapshot** — toolbar Snapshot button prompts user for a custom label; blank or cancel falls back to `'Manual snapshot'`; `postAddSnapshot` throws on HTTP error; `takeSnapshot` returns `boolean`; failure surfaces as an error toast in `handleSnapshot`; (3) **conflict snapshots** — `resolveConflictOverwrite` and `resolveConflictReload` in `useSaveActions` snapshot the relevant version before resolution (`'Remote version before overwrite'`, `'Before remote reload'`); these remain best-effort (failure is silent, never blocks conflict resolution).

**Snapshot UI surfaces (Phase 8)** — Two surfaces expose snapshot data: (1) **ToolsPanel Restore tab** — snapshots for the currently open schedule only; (2) **Library Versions tab** — schedule picker to browse any schedule's snapshots. Both share the same card layout: label is the main title, timestamp + action count are secondary metadata. Both show an `X / 25 snapshots` count badge (`tp-snap-cap` CSS class in `tools-panel.css`); at 25/25 a note "New snapshots replace the oldest." appears. Buttons are unified: Preview / Save As New / Restore / Delete (consistent labels and `tp-snap-del` delete button style in both surfaces). Phase 8 was a UX polish and storage-cap update — not a new versioning architecture. Full autosave redesign, snapshot rename/editing, and styled modal replacement for `confirm()` / `prompt()` are deferred.

**Save As New from snapshot — Blob propagation guard (Phase 8)** — `handleSaveAsNew` in both `ToolsPanel` and `VersionsTab` calls `loadSchedule(newName, {…snap.data, savedAt})` + `setRemoteBaseline(result.savedAt, '')` on the store and uses `router.push` (client-side nav) before navigation. The `loadScheduleFromCloud` guard (`scheduleName === name && remoteBaselineRef.current !== null`) skips the cloud fetch on mount, serving the pre-loaded store data. Prevents blank-schedule flash when the new blob is not yet CDN-readable. Writes the same sessionStorage caches (`rp_recently_added_schedule`, `rp_recently_saved_meta`) as regular `saveAs` for Library consistency.
...
**Tools Panel** — `ToolsPanel.tsx` mounts via `ReactDOM.createPortal(…, document.body)` to escape all ancestor `overflow` contexts. Renders a backdrop + slide-over panel (380px from right on desktop/tablet; 70vh bottom sheet on mobile). Three tabs: (1) Templates — loads from Netlify Blobs, applies rows in-place via `loadSchedule(scheduleName, …) + updateMeta({})` to preserve the current schedule name while marking dirty; (2) Backup — Export JSON with dated filename; (3) Restore — lists snapshots for the current schedule (no picker); Phase 8: snapshot label is the main card title, timestamp + action count are secondary metadata; X/25 count badge; creation failure surfaces an error toast; Save As New pre-loads the store before `router.push` to avoid blank-schedule on first open. Restores via `postSave(force) → loadSchedule → setRemoteBaseline → setSyncStatus('synced')`, with 4-second inline success toast. Snapshot preview overlay uses inline `z-index:10002` to clear the panel's stacking context. Panel z-index: backdrop 10000, panel 10001. Active state `.tp-open` on the Tools toolbar button.
...
**Call Sheet (Phase 10)** — `CallSheetModal.tsx` is opened from `ShareDropdown` (Share → "📄 Call Sheet"). It reads from `useScheduleStore` and assembles a derived document: schedule name, `meta.projectName`, `meta.phase`, `meta.dayNumber`/`meta.totalDays`, date, town, weather, producer, director, camera, general call (first non-sun `timeIn`), and a simplified schedule summary (time + action + location per non-blank row). Call-sheet-specific fields are stored under `meta.callsheet: CallSheetData` (a new optional field on `ScheduleMeta`) and edited inline within the modal — the schedule header and grid remain clean and do not carry these fields. `CallSheetData` fields: `basecamp`, `parking`, `hospital`, `emergency`, `mealNotes`, `safetyNotes`, `specialInstructions`, `notes`. Blank fields are suppressed in both display and print. Contacts are extracted via the same `extractContacts()` pattern as the Contact Sheet and are toggle-gated, defaulting OFF on every modal open.

**LocationField in Call Sheet** — The three location-style Key Information fields (Basecamp, Crew Parking, Nearest Hospital) use a `LocationField` component that wraps `PlacesAutocomplete` (same Google Places proxy used by the main schedule editor). In edit mode the input has Places autocomplete; selecting a suggestion commits the full resolved address. In display mode a `📍` link opens Google Maps directions to the stored address text (`/maps/dir/?api=1&destination=…`). The map pin is hidden in print via `.csh-loc-pin { display: none !important }`. All other Key Information fields (Emergency Contact, Meal/Safety/Special/General Notes) use plain single-line or multi-line inputs. Existing field values are plain text; no structured `locationPlaceId` / `locationLat` / `locationLng` fields are stored in this phase — location assistance is purely input-side.

**Call Sheet print architecture** — A `callsheet-print-only` div is portal-rendered on `document.body` outside the modal DOM, containing `PrintDoc` (branded header, production table, General Call box, Key Information, Contacts, Schedule). `handlePrint()` adds `body.callsheet-printing` to `document.body` before `window.print()` and removes it in the `afterprint` listener. `call-sheet.css` is imported before `print.css` in `globals.css`. Because `print.css` globally hides `td:last-child` (to suppress the editor's delete column), all value cells in Call Sheet's 2-column tables were suppressed. Fix: `.callsheet-print-only td:last-child` at specificity (0,2,1) beats print.css's (0,1,1), restoring all cells. `page-break-inside: avoid` is applied only to Key Information and Contacts (`.csh-pdoc-section-fixed`); the Schedule section deliberately omits it so long timelines break naturally. Section headings use `page-break-after: avoid` to prevent orphaned headings. The `csh-pdoc-td-t` time column carries `white-space: nowrap`. All `td` cells carry `overflow-wrap: anywhere` so long addresses wrap instead of overflowing. Mobile `body { padding-bottom: calc(120px + env(safe-area-inset-bottom)) }` from `inline-patches.css` is zeroed in the print block to prevent a blank trailing page. Normal schedule print, Contact Sheet print, Library, snapshots, editor, and read-only view are entirely unaffected.
...
**Schedule header identity block (Phase 9)** — `HeaderIdentityLine.tsx` renders a two-row identity block at the top of the schedule header. Row 1: `meta.projectName` at 18px bold (`.hi-project`), clickable to edit inline. Row 2: `meta.phase` and Day field at 12px muted (`.hdr-id-row2`), both clickable. ComboInput instances for Project and Phase pass `showAllOnOpen` — all existing options are immediately visible on open; filtering begins only after the user types the first character (tracked via internal `typed` flag in `ComboInput`). Day is a single slash-notation text input (`"1/5"` = Day 1 of 5, `"1"` = Day 1 with totalDays null); `handleDayChange` enforces digits-and-slash only at keystroke level, auto-inserts `/` after the first digit group when typing (not deleting) and positions the cursor after the slash, and limits each segment to 3 digits. Day commit is strict: invalid input (blank day, day=0, day > totalDays) does NOT call `setEditing(null)` — the editor stays open, the input gains the `hi-field-err` red-border class, and an inline `hi-day-err` error message appears. Metadata (`meta.dayNumber`, `meta.totalDays`) is never changed by invalid input. The `daySnapshot` ref captures the current day values at `startEdit` time; pressing Escape calls `revert()` which explicitly restores from that snapshot via `updateMeta` and closes editing. Project/Phase commit on blur or Enter (unchanged from pre-Phase 9 behavior). The identity fields (`meta.projectName`, `meta.phase`, `meta.dayNumber`, `meta.totalDays`) are the same metadata fields used by Library auto-grouping (`buildTree()`), Move To (`move-schedule.js`), and the read-only view identity block in `ScheduleReadView`. Phase 9 did not change `ScheduleMeta` shape, storage schema, Library storage, server functions, or any other editor behavior.
...
**Call Sheet (Phase 10)**: Share → Call Sheet opens `CallSheetModal`; pulls schedule data (name, project/phase/day, date, town, weather, crew, general call, schedule summary) and optional per-schedule `meta.callsheet` fields (Basecamp, Crew Parking, Nearest Hospital, Emergency Contact, Meal Notes, Safety Notes, Special Instructions, General Notes) stored under `ScheduleMeta.callsheet`; blank fields hidden in display and print; Basecamp/Parking/Hospital use `LocationField` with `PlacesAutocomplete` and `📍` map-pin links; Contacts toggle-gated (default OFF); print via `callsheet-print-only` body portal with `body.callsheet-printing` isolation; Roseland-branded print header with logo; Key Information before Schedule in print; Key Information and Contacts avoid page breaks; general call derived from first non-sun row; staggered/group call times and shareable call-sheet URLs deferred
...
---
*Last updated: 2026-05-26 — Phase 10 Call Sheet Foundation complete and deployed. CallSheetModal, LocationField, Call Sheet print architecture, and CallSheetData model documented in Sections 3, 9, and 10. `ScheduleMeta.callsheet?: CallSheetData` added to the data model (additive, old schedules unaffected). No server-side function changes. No changes to schedule editor, Library, snapshots, Contact Sheet, or public view.*

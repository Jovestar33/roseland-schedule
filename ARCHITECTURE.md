...
---
*Last updated: 2026-05-25 — Phase 4 Location Details / Sub-locations complete and tested: stacked inline sub-locations (no modal), SubLocation data model `{ id, loc, locLat, locLng, done, desc }`, Google Places reuse, print-safe controls, Phase 4B contact integration deferred. Post-Phase-4 bug fixes: `useLayoutEffect` in DescTextarea; `type="button"` on all LocationCell buttons; `row.dur === ''` blank-duration guard throughout (replaces `!r.dur` / `durMins <= 0`); `computeTimeOut` now returns Time In for "00:00" duration rows.*

...
const ids=new Set([fixture.target]),control='/private/tmp/roseland-b15-document-fault.json';
const log='evidence/b15-document/cases/network.jsonl';fs.mkdirSync('evidence/b15-document/cases',{recursive:true});
let held=[];const record=v=>fs.appendFileSync(log,JSON.stringify({...v,at:new Date().toISOString()})+'\n');
const server=http.createServer(async(req,res)=>{
 const path=new URL(req.url,'http://127.0.0.1').pathname;
 const headers={'Access-Control-Allow-Origin':req.headers.origin||'http://127.0.0.1:3516','Access-Control-Allow-Headers':req.headers['access-control-request-headers']||'authorization, apikey, content-type, x-client-info, x-supabase-api-version','Access-Control-Allow-Methods':'GET, POST, PUT, DELETE, OPTIONS','Access-Control-Expose-Headers':'content-range'};
 ...
 try{
  ...
  if(matched&&mode.mode==='hold'){held.push(deliver);fs.writeFileSync('/private/tmp/roseland-b15-document-held.json',JSON.stringify({held:held.length,path,id:entry.id}));return;}
  deliver();
 }catch{record({...entry,status:502});if(!res.headersSent)res.writeHead(502,headers);res.end();}
});
const timer=setInterval(()=>{let v;try{v=JSON.parse(fs.readFileSync(control));}catch{}if(v?.release){fs.writeFileSync(control,'{}');const count=held.length;held.splice(0).forEach(f=>f());fs.writeFileSync('/private/tmp/roseland-b15-document-held.json',JSON.stringify({held:0,released:count}));}},100);
server.listen(3515,'127.0.0.1',()=>{fs.writeFileSync('/private/tmp/roseland-b15-document-proxy-process.json',JSON.stringify({pid:process.pid,port:3515}),{mode:0o600});console.log('Candidate-only fixture proxy ready on 3515.');});

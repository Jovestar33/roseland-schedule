...
const path='/Users/johnsammon/Downloads/All-authorized-schedules-2026-09-19 (1).json';
...
assert.equal(file.schedules.length,16);
...
const summary={file:'All-authorized-schedules-2026-09-19 (1).json',records:file.schedules.length,allDocumentsEqualOrdinaryReadback:true,exclusions:file.excludes,sha256:createHash('sha256').update(readFileSync(path)).digest('hex')};
copyFileSync(path,'evidence/b15-document/cases/library-downloaded.json');writeFileSync('evidence/b15-document/cases/library-download-check.json',JSON.stringify(summary,null,2));console.log(JSON.stringify(summary));

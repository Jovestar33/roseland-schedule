...
const ids=new Set(fixture.rows.map(r=>r.id)),control='/private/tmp/roseland-b15-runtime-fault.json';
const log='evidence/b15-runtime/cases/network.jsonl';fs.mkdirSync('evidence/b15-runtime/cases',{recursive:true});
let held=[];const record=v=>fs.appendFileSync(log,JSON.stringify({...v,at:new Date().toISOString()})+'\n');
const server=http.createServer(async(req,res)=>{
 const path=new URL(req.url,'http://127.0.0.1').pathname;
 const headers={'Access-Control-Allow-Origin':req.headers.origin||'http://127.0.0.1:3514','Access-Control-Allow-Headers':req.headers['access-control-request-headers']||'authorization, apikey, content-type, x-client-info, x-supabase-api-version','Access-Control-Allow-Methods':'GET, POST, PUT, DELETE, OPTIONS','Access-Control-Expose-Headers':'content-range'};
 ...
 const allowed=ids.has(payload.target_schedule_id)||['/list_schedule_library','/list_my_organization_access'].some(p=>path.endsWith(p))&&payload.target_organization_id===fixture.organization;
 ...
 const entry={path,id:payload.target_schedule_id??null,after:payload.after_id??null,expected:payload.expected_version??null,mode:matched?mode.mode:null};
 ...
 try{
  ...
  if(matched&&mode.mode==='hold'){held.push(deliver);fs.writeFileSync('/private/tmp/roseland-b15-runtime-held.json',JSON.stringify({held:held.length,path,id:entry.id}));return;}
  deliver();
 }catch{record({...entry,status:502});if(!res.headersSent)res.writeHead(502,headers);res.end();}
});
const timer=setInterval(()=>{let v;try{v=JSON.parse(fs.readFileSync(control));}catch{}if(v?.release){fs.writeFileSync(control,'{}');const count=held.length;held.splice(0).forEach(f=>f());fs.writeFileSync('/private/tmp/roseland-b15-runtime-held.json',JSON.stringify({held:0,released:count}));}},100);
server.listen(3513,'127.0.0.1',()=>{fs.writeFileSync('/private/tmp/roseland-b15-runtime-proxy-process.json',JSON.stringify({pid:process.pid,port:3513}),{mode:0o600});console.log('Candidate-only fixture proxy ready on 3513.');});

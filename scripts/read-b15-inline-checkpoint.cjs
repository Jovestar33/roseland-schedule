...
(async () => {
  ...
  try {
    ...
    assert.equal(data.document_version,5);
    ...
    fs.writeFileSync('evidence/b15-inline/cases/saved-readback.json',JSON.stringify(data,null,2)+'\n');
    const preservation = {retainedSchedules:baseline.schedules.length,versionsNamesAndDocumentsUnchanged:true,newTarget:data.id,newTargetVersion:data.document_version,savedMetadataAndNestedStateVerified:true};
    fs.writeFileSync('evidence/b15-inline/cases/preservation-check.json',JSON.stringify(preservation,null,2)+'\n');

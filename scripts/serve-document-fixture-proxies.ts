...
const data:Record<string,unknown>={'B02 ordinary fictional day':documentFixture(),'B02 long fictional day':documentFixture(32),'B02 empty fictional day':emptyDocumentFixture()};
const tracePath='evidence/b02-documents/browser-request-trace.json';
...
const servers=[3403,3404].map(port=>createServer((req,res)=>{
 const u=new URL(req.url??'/',`http://127.0.0.1:${port}`),legacy=port===3403;
 const record={surface:legacy?'fictional legacy':'authenticated local',method:req.method??'GET',path:u.pathname,result:'local application'};trace.push(record);
 ...
 const upstream=request({hostname:'127.0.0.1',port:3402,path:req.url,method:req.method,headers:req.headers},r=>{
  ...
  res.writeHead(r.statusCode??502,{...headers,'content-security-policy':`default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; connect-src 'self'${legacy?'':' http://127.0.0.1:3401'}; report-uri /__csp_report`});r.pipe(res);
 });upstream.on('error',()=>{res.writeHead(502);res.end('Local app unavailable');});req.pipe(upstream);
}).listen(port,'127.0.0.1'));
console.log('Fictional legacy: http://127.0.0.1:3403/ ; authenticated target: http://127.0.0.1:3404/local-workspace');
const input=createInterface({input:process.stdin});input.on('line',line=>{if(line==='trace'||line==='stop'){writeFileSync('evidence/b02-documents/browser-request-trace.json',JSON.stringify(trace,null,2));console.log('TRACE '+JSON.stringify({requests:trace.length,unexpected:trace.filter(r=>r.result.includes('blocked')).length}));}if(line==='stop'){for(const server of servers)server.close();input.close();}});

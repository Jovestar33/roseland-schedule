...
const tracePath='evidence/b15-parity-remediation/browser-request-trace.json';
...
const servers=[3481,3486].map(port=>createServer((req,res)=>{
 ...
 if(u.pathname.startsWith('/.netlify/')||u.pathname.startsWith('/api/places')||u.pathname.includes('weather')){
  ...
  if(u.pathname==='/.netlify/functions/cms-load'){json(200,JSON.parse(readFileSync('evidence/b15-parity-remediation/observed-theme.json','utf8')));return;}

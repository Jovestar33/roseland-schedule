...
const port=3528,project='roseland-b14-destination-g2';
// Fault injection is explicit; the stable review candidate talks directly to
// the unchanged loopback database when no flag is supplied.
const api=process.argv.includes('--faults')?'http://127.0.0.1:3519':config.API_URL;
const app=spawn(process.execPath,[resolve('node_modules/next/dist/bin/next'),'start','--hostname','127.0.0.1','--port',String(port)],{stdio:'inherit',env:{PATH:process.env.PATH,NODE_ENV:'production',ROSELAND_LOCAL_DOCUMENT_PROVIDERS:'live',GOOGLE_PLACES_KEY:placesKey,NEXT_TELEMETRY_DISABLED:'1',ROSELAND_LOCAL_WORKSPACE:'supabase',ROSELAND_LOCAL_ACCOUNTS:'supabase',ROSELAND_LOCAL_MEMBERSHIP_NOTIFICATIONS:'mailpit',ROSELAND_LOCAL_MAILPIT_URL:'http://127.0.0.1:56524',ROSELAND_LOCAL_SUPABASE_URL:api,ROSELAND_LOCAL_SUPABASE_ANON_KEY:config.ANON_KEY,ROSELAND_LOCAL_ACCOUNT_SERVICE_KEY:config.SERVICE_ROLE_KEY,SUPABASE_PLATFORM_WORKFLOWS_ENABLED:'true',SUPABASE_URL:api,SUPABASE_PUBLISHABLE_KEY:config.ANON_KEY,SUPABASE_SERVICE_ROLE_KEY:config.SERVICE_ROLE_KEY}});
writeFileSync('/private/tmp/roseland-professional-document-process.json',JSON.stringify({launcher:process.pid,server:app.pid,port,project,api,build:readFileSync('.next/BUILD_ID','utf8').trim()}),{mode:0o600});

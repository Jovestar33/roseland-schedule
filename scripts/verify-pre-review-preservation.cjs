...
(async()=>{const clients=[];try{
 ...
 for(const row of baseline.retainedReviewSchedules)assert.deepEqual(await rpc(review,'session_read_schedule',{target_schedule_id:row.id}),row);
 ...
 save('new-records',newRows);const result={checkedAt:new Date().toISOString(),retainedReviewSchedulesUnchanged:18,existingNorthSouth:counts,membershipRowsUnchanged:true,existingProductionPermissionsUnchanged:true,browserMoveVersion:3,movedRestrictionPreserved:true,newRecords:{productions:1,schedules:5,templates:1,restrictions:2},sharingRestriction:share};save('preservation-final',result);console.log(JSON.stringify(result));

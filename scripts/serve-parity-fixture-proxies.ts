...
const data:Record<string,unknown>={'Parity ordinary fictional day':documentFixture(),'Parity long fictional day':documentFixture(32),'Parity empty fictional day':emptyDocumentFixture()};
const tracePath='evidence/legacy-parity/browser-request-trace.json';
...
const servers=[3421,3422].map(port=>createServer((req,res)=>{
 const u=new URL(req.url??'/',`http://127.0.0.1:${port}`),legacy=port===3421;
 ...
 const upstream=request({hostname:'127.0.0.1',port:legacy?3420:3418,path:req.url,method:req.method,headers:req.headers},r=>{
  ...
  res.writeHead(r.statusCode??502,{...headers,'content-security-policy':`default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; connect-src 'self'${legacy?'':' http://127.0.0.1:55521'}; report-uri /__csp_report`});r.pipe(res);
 });upstream.on('error',()=>{res.writeHead(502);res.end('Local app unavailable');});req.pipe(upstream);
}).listen(port,'127.0.0.1'));
console.log('Fictional legacy: http://127.0.0.1:3421/ ; authenticated target: http://127.0.0.1:3422/local-workspace');
const input=createInterface({input:process.stdin});input.on('line',line=>{if(line==='trace'||line==='stop'){writeFileSync('evidence/legacy-parity/browser-request-trace.json',JSON.stringify(trace,null,2));console.log('TRACE '+JSON.stringify({requests:trace.length,unexpected:trace.filter(r=>r.result.includes('blocked')).length}));}if(line==='stop'){for(const server of servers)server.close();input.close();}});
